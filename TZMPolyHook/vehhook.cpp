...
EXTERN_C uintptr_t moenyadress = 0;
EXTERN_C uintptr_t moenyadress2 = 0;
EXTERN_C uintptr_t moenyadress3 = 0;
...
bool  Hook()
{
	for (size_t i = 0; i < guardInfo.size(); i++)
	{
		...
		MessageBox(0, "开始绑定", "guardInfo[i].gpAdress:", MB_SYSTEMMODAL);
		//切换页面上的PAGE_GUARD标志
...
//设置线程的硬件断点
VOID SetThreadHook(HANDLE hThread, CPUSINGLE cpus) {
	CONTEXT ctx;
			...
			if (GetCurrentProcessId() == ThreadInfo->th32OwnerProcessID) {
				...
				for (size_t i = 0; i < exceptionInfo.size(); i++)
					if (exceptionInfo[i].of)
						SetThreadHook(hThread, exceptionInfo[i]);
				//恢复线程
